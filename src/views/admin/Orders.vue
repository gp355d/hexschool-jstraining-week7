...
export default {
  ...
  methods: {
    getOrders: function () {
      const vm = this
      const api = `${process.env.VUE_APP_APIPATH}/${this.uuid}/admin/ec/orders` // 取得訂單資料API
    ...
    setOrderPaid (item) {
      let api = `${process.env.VUE_APP_APIPATH}/${this.uuid}/admin/ec/orders/${item.id}/paid` // 變更付款狀況API，已付款
      if (!item.paid) {
        api = `${process.env.VUE_APP_APIPATH}/${this.uuid}/admin/ec/orders/${item.id}/unpaid` // 變更付款狀況API，未付款
      }
