...
export default {
  ...
  methods: {
    getData: function (pages = 1) {
      const vm = this
      const api = `${process.env.VUE_APP_APIPATH}/${this.uuid}/admin/storage?page=${pages}` // 取得storage的檔案API
    ...
    deleteimgData: function () {
      const vm = this
      const api = `${process.env.VUE_APP_APIPATH}/${this.uuid}/admin/storage/${this.tempData.id}` // 刪除storage的檔案API
